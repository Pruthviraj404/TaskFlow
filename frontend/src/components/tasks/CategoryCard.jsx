...
// Sub-component for individual tasks within the Category Card
  ...
  return (
    <div className="border-b border-gray-50 last:border-0 overflow-hidden">
      ...
      <div 
        ...
        className="flex items-center gap-4 px-10 py-3 hover:bg-gray-50/30 group cursor-pointer transition-colors"
      >
        <button 
          onClick={(e) => { 
            e.stopPropagation(); 
        ...
        <div className="flex items-center gap-3">
          {isOverdue && (
            <span className="text-[9px] font-black text-red-500 uppercase tracking-tight bg-red-50 px-1.5 py-0.5 rounded">Overdue</span>
          )}
          <ChevronDown 
            size={14} 
            className={`text-gray-300 transition-transform duration-300 ${isTaskExpanded ? 'rotate-180' : ''}`} 
          />
        </div>
      </div>
      ...
      {isTaskExpanded && (
        <div className="px-10 pb-4 pt-1 bg-gray-50/20 animate-in slide-in-from-top-1 duration-200">
          <div className="pl-9 space-y-3">
            {/* Description */}
            <div className="flex gap-2">
              <AlignLeft size={14} className="text-gray-400 mt-1 shrink-0" />
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Description</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[8px] font-bold text-gray-400 uppercase">Due Date</p>
                  <p className="text-[11px] font-bold text-gray-600">{task.due_date}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <AlertCircle size={13} className="text-gray-400" />
                <div>
                  <p className="text-[8px] font-bold text-gray-400 uppercase">Priority</p>
                  <p className={`text-[11px] font-bold capitalize ${
                    task.priority === 'high' ? 'text-red-500' : 'text-gray-600'
                  }`}>
  ...
  return (
    <div className={`bg-white rounded-2xl border transition-all duration-300 ${
      ...
      <div 
        ...
        className="p-5 flex items-center justify-between cursor-pointer hover:bg-gray-50/50 transition-colors rounded-t-2xl"
      >
      ...
      {isExpanded && (
        <div className="border-t border-gray-50 bg-white animate-in slide-in-from-top-2 duration-300 pb-2">
          {tasks.length > 0 ? (
            ...
            <div className="p-8 text-center text-gray-400 text-xs italic">No tasks yet in this category.</div>
          )}
        </div>
      )}
    </div>
